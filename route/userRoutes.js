...
// router.route('/signup').get(getSignUp).post(checkNotExisting, newUser);
...
router.route('/product-details/:id').get(getProductDetails).post(axiosUserLoginCheck, axiosCheckBlockedUser, postComment);
...
router
  ...
  .post(userLoginCheck, checkBlockedUser, addAddress);
